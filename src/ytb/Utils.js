...
class Utils {
  ...
  static numberFormatter(number, digits) {
    ...
    return (
      (number / def[i].value).toFixed(digits).replace(rx, '$1') + def[i].symbol
    );
  }
  ...
  static extractPlaylistUrls(infoQueryResult) {
    ...
    let alreadyDone = [];
    if (
      infoQueryResult.entries == null ||
      infoQueryResult.entries.length === 0
    ) {
  ...
  static detectInfoType(infoQueryResult) {
    ...
    if (infoQueryResult.is_live != null && infoQueryResult.is_live === true)
      return 'livestream';
    if (infoQueryResult._type != null && infoQueryResult._type === 'playlist')
      return 'playlist';
    ...
    for (let dataFormat of metadata.formats) {
      ...
      if (!detectedFormats.includes(format.getDisplayName())) {
        for (const dataFormat of metadata.formats) {
          ...
          if (
            dataFormat.height !== format.height ||
            dataFormat.fps !== format.fps
          )
            continue;
  ...
  static generatePlaylistMetadata(query) {
    ...
    for (const entry of query.entries) {
      ...
      if (entry.url == null) url = entry.webpage_url;
        ...
        url =
          entry.ie_key != null && entry.ie_key === 'Youtube'
            ? 'https://youtube.com/watch?v=' + entry.url
            : entry.url;
